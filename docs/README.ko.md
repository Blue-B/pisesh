...
TUI가 실제로 어떻게 보이는지 화면별로. (데모 데이터 — 실제 세션이 아닙니다.)

**메인 목록** — 하이라이트된 행이 선택된 상태, `Tab`으로 탭을 순환합니다. 초록 `[NOW]` 배지는 지금 띄운 pi 세션, 청록 `✎`는 직접 이름을 바꾼 세션 표시. 한글 제목도 컬럼 정렬이 유지됩니다:

<p align="center"><img src="../assets/screen-list.png" alt="pisesh 메인 목록 — Favorites 탭, Today / Here / All 탭, [NOW] 배지, 이름 바꾼 세션" width="100%"></p>

**`e` — 세션 이름 바꾸기.** 첫 prompt는 장기 스레드 제목으로는 별로죠. `e`로 원하는 이름을 지정하면 override로 저장되고(세션 jsonl은 건드리지 않음) 목록에 `✎` 마커가 붙습니다:

<p align="center"><img src="../assets/screen-rename.png" alt="pisesh 이름 편집 패널 — 커스텀 제목 지정" width="100%"></p>

**`p` — 작업 디렉터리 다시 지정.** 방향키 디렉터리 브라우저로 resume 시 pi가 실제로 `cd`할 경로를 고릅니다. 이 경로가 `Here` 탭의 필터 기준이 됩니다. `s`로 하이라이트된 디렉터리를 확정:

<p align="center"><img src="../assets/screen-cwd.png" alt="pisesh cwd 브라우저 — resume / Here 디렉터리를 고르는 방향키 피커" width="100%"></p>

**`Here` 탭**은 pisesh를 띄운 디렉터리와 cwd가 일치하는 세션만 보여줍니다 — 프로젝트 안에서 열면 그 프로젝트 스레드만, 홈 디렉터리 임시 세션은 스크롤로 지나칠 필요가 없습니다.
...
| 현재 프로젝트 세션만 보기                     | `Here` 탭 — pisesh를 띄운 디렉터리와 cwd가 일치하는 세션만 필터    |
| 세션이 재개될 위치 고치기                     | `p` 로 방향키 디렉터리 브라우저 — pi가 `cd`할 cwd 지정             |
...
| `e`                         | 이름 편집 — 커스텀 제목 지정 (목록에 `✎` 표시)                |
| `p`                         | cwd 편집 — 방향키 디렉터리 브라우저; resume / `Here` 기준 경로 |
...
| ✅   | `Here` 탭 — pisesh를 띄운 디렉터리와 일치하는 세션만 필터                |
| ✅   | 인라인 rename (`e`) — 첫 prompt를 덮어쓰는 커스텀 제목                       |
| ✅   | cwd 편집 (`p`) — 방향키 디렉터리 브라우저로 resume / `Here` 경로 지정      |
...
- [pi-coding-agent](https://github.com/earendil-works/pi) by [@mariozechner](https://github.com/mariozechner) — 에이전트와 익스텐션 API 가 `/sesh` 를 가능하게 함.
- [interactive-shell 예제 익스텐션](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/examples/extensions/interactive-shell.ts) — `ui.custom` + `tui.stop` TTY 인수 패턴 참조.
